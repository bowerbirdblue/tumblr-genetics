...
def submitWidgets():
    ...
    preLab = tk.Label(rightFrame, text=preText, font=(font, 15), wraplength=375, justify="left")
    preLab.pack(side="top", anchor="w", fill="x")
    print(f"\nSubmitted Text: {preText}")
    tk.Label(rightFrame, text="Regex results:", font=(font, 17), relief="groove").pack(side="top", anchor="w")
    seqLab = tk.Label(rightFrame, text=mySeq, font=(font, 15), wraplength=375, justify="left")
...
def blasting():
    ...
    print(f"\nClosest Match: {org}")
    matchLab = tk.Label(rightFrame, text=org, font=(font, 15), wraplength=375, justify="left")
    matchLab.pack(side="top", anchor="w")
'''
SUBMISSION FUNCTION
-
...
def appWin():
    global textInput
    #scroll = tk.Scrollbar(rightFrame, orient="vertical")
    ...
    submit = tk.Button(subFrame, font=(font, 30), text="Submit", command=submitFunc, padx=0,pady=0, height=60, width=400) # creates submission button
    submit.pack(anchor="center") # places submission button
    '''RIGHT SIDE'''
